...
class Database:
    ...
    def init(self):
        #Tables
        self._db.execute('create table IF NOT EXISTS users (ID INTEGER PRIMARY KEY NOT NULL, Name text, Email text, Password text, Auth_Type text, Account_Type text, Keys text, Key_Distributed INTEGER, Grp INTEGER)')
        self._db.execute('create table IF NOT EXISTS groups (ID INTEGER PRIMARY KEY NOT NULL, Name text, Internal text, External text, Used_Octets text, Rule INTEGER)')
